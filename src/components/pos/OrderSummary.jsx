...
function OrderSummary({ 
    ...
    // Solo mantenemos estado de UI (Modal)
                ...
                {totals.savings > 0 && (
                    <div className="flex flex-col items-end animate-in fade-in slide-in-from-right-2">
                        ...
                        <span className="text-sm font-bold text-emerald-600 flex items-center gap-1 bg-emerald-50 px-2.5 py-0.5 rounded-lg border border-emerald-100">
                            <Sparkles size={12} fill="currentColor" /> ${totals.savings.toFixed(2)}
                        </span>
                    </div>
                )}
            </div>
            ...
            <div className="flex-grow overflow-y-auto p-4 custom-scrollbar space-y-3 bg-gray-50/30">
                {cartItems.length === 0 ? (
                    ...
                    cartItems.map((item) => {
                        ...
                        return (
                            <div key={item.id} className={`flex flex-col p-3 bg-white rounded-2xl border transition-all duration-200 group relative overflow-hidden ${borderClass}`}>
                                {/* ... CONTENIDO DEL ITEM (IGUAL QUE ANTES) ... */}
                                {/* Solo asegúrate de usar las variables desestructuradas arriba (originalPrice, unitPrice, etc.) */}
                                <div className="flex items-center">
                                        ...
                                        {isDiscountActive && (
                                            <div className={`absolute bottom-0 left-0 right-0 h-3 ${themeBg} flex items-center justify-center`}>
                                                <span className="text-[8px] font-bold text-white flex items-center gap-0.5 leading-none">
                                                    {isBulkType ? <Layers size={7} /> : <Percent size={7} />} -{discountPct}%
                                                </span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex-grow min-w-0 mr-2">
                                        <p className="font-bold text-gray-700 text-sm truncate leading-tight mb-1">{item.name}</p>
                                        <div className="flex flex-col">
                                            {isDiscountActive ? (
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[10px] text-gray-400 line-through decoration-gray-300">${originalPrice.toFixed(2)}</span>
                                                    <span className={`text-sm font-black flex items-center gap-1 ${themeColorText}`}>${unitPrice.toFixed(2)}</span>
                                                </div>
                                            ) : (
                                                <p className="text-sm font-bold text-gray-600">${originalPrice.toFixed(2)}</p>
                                            )}
                                        </div>
                                    </div>
                                    {/* Controles */}
                                    <div className="flex items-center gap-1 bg-gray-50 rounded-xl p-1 border border-gray-200">
                                        <button onClick={() => onUpdateQuantity(item.id, -1)} className="w-6 h-6 flex items-center justify-center rounded-lg text-gray-400 hover:bg-white hover:text-red-500 font-bold transition-all">-</button>
                                        <span className="text-xs font-extrabold text-gray-700 w-6 text-center">{item.quantity}</span>
                                        <button onClick={() => onUpdateQuantity(item.id, 1)} disabled={isMaxStock} className="w-6 h-6 flex items-center justify-center rounded-lg text-gray-400 hover:bg-white hover:text-green-600 font-bold transition-all disabled:opacity-30">+</button>
                                    </div>
                                    {/* Eliminar */}
                                    <button onClick={() => onRemoveItem(item.id)} className="ml-2 text-gray-300 hover:text-red-500 transition-colors p-1.5 hover:bg-red-50 rounded-xl">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                                {/* Avisos */}
                                {isNearDiscount && (
                                    <div className="mt-2 bg-amber-50 border border-amber-100 rounded-lg p-1.5 flex items-center justify-between animate-in fade-in slide-in-from-top-1">
                                        ...
                                        <button onClick={() => onUpdateQuantity(item.id, minQty - item.quantity)} className="text-[9px] bg-white border border-amber-200 text-amber-700 px-2 py-0.5 rounded shadow-sm hover:bg-amber-100 font-bold uppercase">Agregar</button>
                                    </div>
                                )}
                            </div>
                        );
            ...
            <div className="p-6 border-t border-gray-100 bg-white flex-shrink-0 space-y-4 shadow-[0_-10px_40px_rgba(0,0,0,0.03)] z-20">
                <div className="space-y-1.5">
                    <div className="flex justify-between text-sm text-gray-500 font-medium">
                        <span>Subtotal</span>
                        <span className="text-gray-700 font-bold">${totals.subtotal.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm text-gray-500 font-medium">
                    ...
                    <div className="flex justify-between items-center pt-3 mt-2 border-t border-dashed border-gray-200">
                        ...
                        <div className="text-right">
                            <span className="font-black text-3xl text-pink-600 tracking-tight">${totals.total.toFixed(2)}</span>
                        </div>
                    </div>
                </div>
                ...
                >
                    {isProcessing ? <><Loader2 className="animate-spin" /> Procesando...</> : <>Procesar Venta</>}
                </button>
            </div>
