...
import { X, RefreshCw, Save, Tag, ToggleLeft, ToggleRight, AlertCircle } from 'lucide-react';
...
function CategoryModal({ isOpen, onClose, onSubmit, categoryToEdit }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // El onSubmit llama al servicio que ya usa Axios
            await onSubmit(formData);
            // El cierre del modal lo maneja el padre si todo sale bien, 
            // o podemos cerrarlo aquí si el onSubmit no lanza error.
            onClose(); 
        } catch (err) {
            console.error(err);
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            ...
                            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100">
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-bold transition-all active:scale-95 ${
                                        formData.isActive 
